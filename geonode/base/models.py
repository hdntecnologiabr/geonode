...
class EmbrapaAuthorsQuerySet(MP_NodeQuerySet):
    """QuerySet to automatically create a root node if `depth` not given."""

    def create(self, **kwargs):
        if 'name' not in kwargs:
            return self.model.add_root(**kwargs)
        return super(EmbrapaAuthorsQuerySet, self).create(**kwargs)


class EmbrapaAuthorsManager(MP_NodeManager):

    def get_queryset(self):
        return EmbrapaAuthorsQuerySet(self.model).order_by('name')

class Embrapa_Authors(TagBase, MP_Node):

    # afiliacao exemplo: "CNPGC"
    afiliacao = models.CharField(max_length=50)

    # autoria exemplo: "OLIVEIRA, H. S. de"
    autoria = models.CharField(max_length=300)

    node_order_by = ['name']

    objects = EmbrapaAuthorsManager()
...
class Embrapa_Authors_ResourceBase(ItemBase):
    content_object = models.ForeignKey('ResourceBase', on_delete=models.CASCADE)
    tag = models.ForeignKey('Embrapa_Authors', 
        related_name='embrapa_autores', on_delete=models.CASCADE)
...
class _EmbrapaAuthorTagManager(_TaggableManager):
    def add(self, *tags):
        str_tags = set([
            t
            for t in tags
        ...
        for new_tag in str_tags - set(t.name for t in existing):
            if new_tag:
                new_tag = escape(new_tag)
                tag_objs.add(Embrapa_Authors.add_root(name=new_tag))
...
class ResourceBase(PolymorphicModel, PermissionLevelMixin, ItemBase):
    ...
    embrapa_autores = TaggableManager(
        ...
        blank=True,
        help_text=autores_help_text,
        manager= _EmbrapaAuthorTagManager)
